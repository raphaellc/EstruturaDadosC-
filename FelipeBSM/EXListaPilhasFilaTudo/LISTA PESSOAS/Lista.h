...
void Lista<T>::inserirInicioLista(No<T>* no)
{
	...
	{   
		...
		this->lista = no;
		this->lista->proximo = this->lista_aux;
		this->tam++;
	}
...
void Lista<T>::esvaziaLista()
{
	if (this->lista != nullptr) {
		...
		while (this->lista_aux != nullptr) {
			//primeiro passo: atualizar lista_aux para o próximo n
...
T* Lista<T>::removePrimeiroNo()
{
	...
	{   //aqui ele pega o valor de lista e passa para lista aux para não perder a info
		//depois diz que o novo "item" é o proximo da antiga dela
		//depois salva a informação em uma variavel e deleta lista_aux;
		//por fim se diminui o tamnho da lista;
		//LEMBRETE: FOI ASSIM QUE EU ENTENDI, POSSO ESTAR ENGANADO...
