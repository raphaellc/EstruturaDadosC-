...
public:
	...
	void esvaziaLista();
	bool contemNaLista(const No<T> no);
	bool removerNo(const No<T> no);
	T*obtemInfo(const No<T> no);
protected:
	int tam;
};
...
template<class T>
inline bool Lista<T>::contemNaLista(const No<T> no)
{
	if (lista_aux->info == no.info) {
		return true;
	}
	else if (no->info != lista_aux->info && lista_aux->proximo !=nullpt) {
